...
import PickupArticle from "../components/pickup-articles";
...
export const getStaticProps = async () => {
  ...
  const topRes = await fetch(
    `https://newsapi.org/v2/top-headlines?country=kr&pageSize=${pageSize}&apiKey=`
  );
  ...
  const weatherRes = await fetch(
    `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${lon}&units=metric&exclude=${exclude}&appid=`
  );
  ...
  const pickupRes = await fetch(
    `https://newsapi.org/v2/everything?q=${keyword}&language=kr&sortBy=${sortBy}&pageSize=${pickupPageSize}&apiKey=`
  );
